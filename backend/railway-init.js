...
async function initializeRailwayDeployment() {
    ...
    try {
        ...
        if (labUser) {
            ...
            const insertGig = db.prepare('INSERT OR IGNORE INTO gigs (title, description, location, lab_id) VALUES (?, ?, ?, ?)');
            
            for (const gig of testGigs) {
                const result = insertGig.run(gig.title, gig.description, gig.location, gig.lab_id);
                if (result.changes > 0) {
